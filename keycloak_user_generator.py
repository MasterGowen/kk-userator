...
try:
    from config import Config, load_config, get_default_config, ConfigValidationError
except ImportError:
    print("Ошибка: не найден модуль config.py")
    print("Убедитесь, что config.py находится в той же директории")
    sys.exit(1)
...
class KeycloakUserGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            realm_name: Имя realm (по умолчанию из конфигурации)
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Успешное подключение к realm: {realm_info.get('realm', 'unknown')}"
            )
    ...
    def _generate_user_data(
        ...
        return {
            ...
            'password': password,
            'email': email,
            'firstName': self.config.user.first_name,
            'lastName': last_name,
            'enabled': True,
            'group': self.config.user.group_name
        }
    ...
    def generate_users(
        ...
        self.logger.info(
            f"Начало генерации {count} пользователей "
            f"(начиная с {start_number})"
        )
        ...
        for i in range(start_number, start_number + count):
            ...
            if self.dry_run:
                ...
                self.logger.info(
                    f"[DRY-RUN] Будет создан: {user_data['username']} | "
                    f"{user_data['email']}"
                )
                ...
                if self._create_user(
                    ...
                    created_users.append(user_data)
                    self.logger.info(
                        f"Создан: {user_data['username']} | {user_data['email']}"
                    )
                else:
                    self.logger.error(
                        f"Не удалось создать: {user_data['username']}"
                    )

            # Небольшая задержка для предотвращения rate-limiting
            if not self.dry_run and i % 10 == 0:
                self.logger.info(
                    f"Прогресс: {i - start_number + 1}/{count}"
                )
...
Примеры использования:
  python keycloak_user_generator.py                    # Интерактивный режим
  python keycloak_user_generator.py --count 50         # 50 пользователей
  python keycloak_user_generator.py --dry-run          # Режим проверки
  python keycloak_user_generator.py --no-interactive   # Переменные окружения
...
def load_application_config(args: argparse.Namespace) -> Config:
    ...
    try:
        ...
        if not config_path:
            config_path = os.environ.get('KEYCLOAK_CONFIG')
        ...
        print("Используйте --config для указания пути или создайте config.yaml")
        sys.exit(1)
...
def main():
    ...
    # Сообщаем о загруженной конфигурации
    ...
    # Код выхода
    sys.exit(1 if generator.stats['errors'] > 0 else 0)
